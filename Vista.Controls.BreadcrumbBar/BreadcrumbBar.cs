...
namespace Vista.Controls {
  ...
  this.DropArrowBounds ) != Rectangle.Empty;
    ...
    protected override void OnPaint ( PaintEventArgs e ) {
      ...
      g.Clear ( this.IsMouseOver || this.IsNodeDropVisible ? this.HoverBackColor : this.BackColor );
    ...
    private void DrawDropArrow ( Graphics g ) {
      ...
      if ( IsOverDropArrow ) {
        ...
        using ( Brush brsh = new SolidBrush ( IsMouseOver || this.IsNodeDropVisible ? HoverBackColor : BackColor ) ) {
    ...
    public class BreadcrumbBarNodeCollection : ControlCollection {
      ...
      public override void Clear ( ) {
        lock ( nodes ) {
          foreach ( var item in nodes ) {
            if ( item.Equals ( ( this.Owner as BreadcrumbBar ).Root ) ) {
              continue;
            } else {
